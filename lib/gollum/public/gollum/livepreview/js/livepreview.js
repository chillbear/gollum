...
var applyTimeout = function () {
  ...
  */
  ...
  var getEditorContentHeight = function() {
    return editor.renderer.layerConfig.maxHeight;
  }



  var syncPreview = function(scrollPos) {
    // based off of realms wiki
    // console.log('scroll pos:' + scrollPos);
    // console.log('editor scroll height:' + editor.container.scrollHeight);
    // console.log('editor height:' + $(editor.container).height());
    // console.log('editor scroll top:' + editor.container.scrollTop);
    // console.log('editor scroll top func:' + editor.session.getScrollTop());
    var $preview = $(preview);
    ...
    var editorVisibleHeight = $(editorContainer).height();
    var editorScrollRange = getEditorContentHeight() - editorVisibleHeight;
    // console.log(editorScrollRange);
    var scrollFactor = scrollPos / editorScrollRange;


    var previewScrollRange = preview.scrollHeight  - $preview.height();
    // var previewScrollRange = preview.scrollHeight;
    // console.log('first row:' + editor.getFirstVisibleRow());
    // console.log('scroll range:' + editorScrollRange);
    scrollFactor = Math.min(1, Math.max(0, scrollFactor));
    // console.log('scroll factor:' + scrollFactor);
    // console.log('preview range:' + previewScrollRange);

    var previewScrollTo = scrollFactor * previewScrollRange
    // console.log(previewScrollTo)
    // console.log('layer config')
    // console.log(editor.renderer.layerConfig)

    $preview.scrollTop(previewScrollTo);
  }

  // console.log(editorSession);


  editorSession.addEventListener('changeScrollTop', function(x) {
    syncPreview(x);
  });

});

// $.editorSession.on('changeScrollTop', function(scroll) {
//   console.log(scroll);
// });
